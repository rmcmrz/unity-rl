...
namespace Apex.Steering.Behaviours
{
	...
	public class AgentBehaviourTest : ExtendedMonoBehaviour, IHandleMessage<UnitNavigationEventMessage>
	{
		...
		float[] getState(Vector3 position)
		{
	...
	*/
		...
		void OnGUI() {
			if (GUI.Button (new Rect (10, 70, 50, 30), "Up"))
				MoveNext (false, 0);

			if (GUI.Button (new Rect (10, 120, 50, 30), "Down"))
				MoveNext (false, 1);

			if (GUI.Button (new Rect (80, 70, 50, 30), "Left"))
				MoveNext (false, 2);

			if (GUI.Button (new Rect (80, 120, 50, 30), "Right"))
				MoveNext (false, 3);
		}
		...
		private void Awake()
		{
			...
			visitedReward = (float) - consumeReward / 5;
		...
		private void MoveNext(bool append,int action)
		{
...
*/
				...
				takeBestAction(futureActions,action);
		...
		private void takeRandomAction(List<IGridCell> futureStates)
		{
			...
			float[] featureValues = getState (futureStates [randomAction].position);
		...
		private void takeBestAction(List<IGridCell> futureStates,int action)
		{
			...
			for (int i = 0; i < allFutureValues.Length; i++) {
				...
				currentStateVector = getState (futureStates [i].position);
